...
if [ "$constraintType" == "linear" ]; then
	for i in `seq 1 $loopcount`;
		do
			...
			echo "negcounter$i =Int('negcounter$i')"
			echo "s.add(negcounter$i >= 0)"
			echo "s.add(negcounter$i <= 1)"
		done
	printf "s.add( "
	printf "counter$i * $i + negcounter$i * $(($i * -1)) "
...
s0= Int('s0')"
	for i in `seq 1 $loopcount`;
		do
			echo "counter$i=Int('counter$i')"
			echo "s.add(counter$i==If(s$(($i-1)) == 50,counter$(($i-1)) + $i, counter$(($i-1)) - $i))"
			echo "s$i=Int('s$i')"
		done
	echo ""
