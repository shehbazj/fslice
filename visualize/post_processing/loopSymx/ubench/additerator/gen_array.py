...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print "# 1st argument  = postCondition = randomIntValue\n # 2nd argument = end_iterator value = array size"
        sys.exit()
...
#    itOperations =   # {'add', 'sub'}
    ...
    generation_begin = 1
    generation_end = 1000
    mutation_chromosome_count = generation_end / 10
    mutation_gene_count = it_end / 2 
    age_limit = 10
    mutationTrigger = 10
...
#                    fitness[parent2Index] = childFitness
 ...
 #               print "BEFORE chromosome ", mutation_chromosome_index, " -> " , mutation_chromosome 
                for mutation_gene_index in random.sample(range( 0 , arrsize - 1), mutation_gene_count):
                    if mutation_chromosome[mutation_gene_index] != 0: 
                        mutation_chromosome[mutation_gene_index] = 0
                    else: 
                        mutation_chromosome[mutation_gene_index] = mutation_gene_index
