...
uint64_t pathCounter::assignTaint(Value *V)
{
	...
	if (ConstantInt *CI = dyn_cast<ConstantInt>(V)){
		if(CI->getBitWidth() <=32){
			D(std::cerr << __func__ << "():bit width less than 32" << std::endl;)
...
void pathCounter::instrumentModule(Module *M_)
{
	...
	for (auto &F_ : M->functions()) {
		...
		if (!F->isDeclaration()) {
			runOnFunction();
			//std::cerr << F->getName().data() << "->" << branchCount << std::endl;
		}
	}
}
...
bool pathCounter::runOnModule(Module &M_) {
	...
	instrumentModule(&M_);
	std::cerr << "Branch Count " << branchCount << std::endl;
